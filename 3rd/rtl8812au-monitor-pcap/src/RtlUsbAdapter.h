...
public:
  ...
  // Same read, but reports the libusb_bulk_transfer return code so the caller can tell
  // a benign timeout (silence) from a fatal LIBUSB_ERROR_NO_DEVICE (adapter unplugged).
  std::vector<Packet> infinite_read(int &usb_rc);
