...
// Rate-limit a high-frequency stderr diagnostic to ~1/sec so a headless run isn't
// flooded (e.g. during wfb session-key sync or heavy packet loss). Each call site
// gets its own static counters; the number suppressed since the last print is
// appended when it next fires. Logging only — does not touch the FEC/decrypt path.
#define WFB_RL_FPRINTF(...)                                                                             \
    do {                                                                                                \
        static std::chrono::steady_clock::time_point _rl_last {};                                       \
        static uint64_t _rl_dropped = 0;                                                                \
        auto _rl_now = std::chrono::steady_clock::now();                                                \
        if (_rl_last.time_since_epoch().count() == 0 || _rl_now - _rl_last >= std::chrono::seconds(1)) { \
            fprintf(stderr, __VA_ARGS__);                                                               \
            if (_rl_dropped) {                                                                          \
                fprintf(stderr, "  (+%" PRIu64 " more suppressed)\n", _rl_dropped);                     \
            }                                                                                           \
            _rl_dropped = 0;                                                                            \
            _rl_last = _rl_now;                                                                         \
        } else {                                                                                        \
...
Aggregator::Aggregator(const std::array<uint8_t, 64> &keypair, uint64_t epoch, uint32_t channel_id,
    ...
    memcpy(rx_secretkey, keypair.data(), crypto_box_SECRETKEYBYTES);
    memcpy(tx_publickey, keypair.data() + crypto_box_SECRETKEYBYTES, crypto_box_PUBLICKEYBYTES);
}
...
void Aggregator::process_packet(
    ...
    if (crypto_aead_chacha20poly1305_decrypt(
        ...
        != 0) {
        WFB_RL_FPRINTF("Unable to decrypt packet #0x%" PRIx64 "\n", be64toh(block_hdr->data_nonce));
        count_p_dec_err += 1;
        return;
    }
...
void Aggregator::send_packet(int ring_idx, int fragment_idx) {
    ...
    if (packet_size > MAX_PAYLOAD_SIZE) {
        WFB_RL_FPRINTF("Corrupted packet %u\n", seq);
        count_p_bad += 1;
