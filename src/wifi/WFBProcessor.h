...
public:
    ...
    // Key-from-memory ctor: keypair = 64 bytes [rx_secretkey(32)][tx_publickey(32)] (gs.key layout).
    // Used by the sealed in-memory key feed (Step 7) so the key never touches disk.
    Aggregator(const std::array<uint8_t, 64> &keypair, uint64_t epoch, uint32_t channel_id,
               const DataCB &cb = nullptr);
    ~Aggregator();
    virtual void
    process_packet(const uint8_t *buf, size_t size, uint8_t wlan_idx, const uint8_t *antenna, const int8_t *rssi);
