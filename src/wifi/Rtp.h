...
// `PACKED` is a GCC-style struct-packing attribute. Upstream relied on it being
// defined transitively by a header pulled in via the Qt GUI build; the headless
// fork dropped <QUdpSocket>, so define it here to stay toolchain-independent. On
// MSVC the struct is already laid out via the #pragma pack(push,1)/pop below, so
// the attribute is a no-op.
#ifndef PACKED
#if defined(_MSC_VER)
#define PACKED
#else
